...
(defn delete-cons
  "Delete `elt` from `xx` (a cons thingy)."
  [elt xx]
  (cond (empty? xx) nil
        (= elt (first xx))

           (delete-cons nil (rest xx))


        :else (cons (first xx) (delete-cons elt (rest xx)))))

(defn delete
  "Delete `elt` from `xx`."
  [elt xb]
  (def xx (cons-to-list (last (first xb))))
  (List. (list-to-cons (delete-cons elt xx)) (count (delete-cons elt xx))))
...
(defn delete-all-cons
  ...
  (cond (empty? xx) nil
        (= elt (first xx))
           (delete-all-cons elt (rest xx))
        :else (cons (first xx) (delete-all-cons elt (rest xx)))))

(defn delete-all
  "Delete all occurrences of `elt` from `xx`."
  [elt xa]
  (def xx (cons-to-list (last (first xa))))
  (List. (list-to-cons (delete-all-cons elt xx)) (count (delete-all-cons elt xx))))
