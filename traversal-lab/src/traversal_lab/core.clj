...
(defn get-backtrack "Given a node and a direction, check if it is part of the backtrack."
  [node dir]
  (case dir
    :w (bit-test node 15)
    :s (bit-test node 14)
...
(defn set-backtrack "Given a node and a direction, set the backtrack status."
  [node dir up]
  (let [op (if up bit-set bit-clear)]
    (case dir
      :w (op node 15)
...
(defn solution?
  "Check if cell `[r c]` in maze `m` is part of the solution path."
  [m [r c]]
  (let [node (get-2d m r c)]
    (or  (get-solution node :n)
         (get-solution node :s)
         (get-solution node :e)
         (get-solution node :w))))

;; i need to reverse what this Returns
(defn unvisted-cell?
  "Check if cell `[r c]` in maze `m` is part of the solution or backtrack path."
  [m [r c]]
  (let [node (get-2d m r c)]
    (not (or  (get-solution node :n)
              (get-solution node :s)
              (get-solution node :e)
              (get-solution node :w)
              (get-backtrack node :n)
              (get-backtrack node :s)
              (get-backtrack node :e)
              (get-backtrack node :w)))))

(defn accessible?
  "Checks to see if a location is accessible by a single move from another location"
  [maze loc-a loc-b]
  (let [node-a (get-2d maze loc-a)
        node-b (get-2d maze loc-b)]
    (not (get-wall node-a (get-direction loc-a loc-b)))))

(defn get-path-neighbors
  "Gets all neighbors of the location that doesnt have a wall blocking"
  [maze loc]
  (let [adjacent (get-neighbors maze loc)]
    (filter #(accessible? maze loc %) adjacent)))

(defn get-unvisted-neighbors
  "Get the neighbors of `loc` and return the ones that are not part of solution or backtrack."
  [m loc]
  (filter #(unvisted-cell? m %) (get-path-neighbors m loc)))

(defn get-backtrack-target
  "Get the neighbors of `loc` and returns the first of the ones that are part of solution."
  [m loc]
  (first (filter #(solution? m %) (get-path-neighbors m loc))))

(defn solve-maze-dfs
  "Solves a maze using the DFS algorithm.  It should set the `solution` bshow
  its of the maze
  to show the path, and return the corresponding maze."
  [maze start-loc goal-loc]
  (cond (= start-loc goal-loc) maze
        (>= 0 (count (get-unvisted-neighbors maze start-loc)))
              (let [current start-loc
                    target  (get-backtrack-target maze start-loc)
                    dir     (get-direction current target)
                    m       (-> maze (swap-2d (current 0) (current 1) #(set-backtrack % dir true))
                                     (swap-2d (target  0) (target  1) #(set-solution % (reverse-direction dir) false)))]
                (solve-maze-dfs m target goal-loc))

        :else (let [current start-loc
                    target  (first (get-unvisted-neighbors maze start-loc))
                    dir     (get-direction current target)
                    m       (-> maze (swap-2d (current 0) (current 1) #(set-solution % dir true)))]
                (solve-maze-dfs m target goal-loc))
        ))
